...
const Home = () => {
  ...
  const { me, logInLoading, logoutLoading, logoutDone, logInDone } = useSelector((state) => state.user);
  const {
    mainPosts,
    hasMorePosts,
    loadPostsLoading,
        ...
        <CellMeasurer key={key} cache={cache} parent={parent} columnIndex={0} rowIndex={index}>
          {({ measure, registerChild }) => (
            <div ref={registerChild} style={style} onLoad={measure}>
              ...
              <PostCard post={mainPosts[index]} setClickedCommentForm={setClickCommentForm} />
            </div>
          )}
        </CellMeasurer>
      );
          ...
          {({ width }) => (
            <WindowScroller>
              {({ height, isScrolling, onChildScroll, scrollTop }) => (
                <InfiniteLoader loadMoreRows={loadMoreRows} isRowLoaded={isRowLoaded} rowCount={1000}>
